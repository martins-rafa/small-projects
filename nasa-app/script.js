...
const removedConfirmed = document.querySelector(".removed-confirmed");
...
function saveFavorite(itemUrl) {
  ...
  resultsArray.forEach((item) => {
    if (item.url.includes(itemUrl) && !favorites[itemUrl]) {
      ...
      // Show Save Confirmation fo 2seconds
...
function removeFavorite(itemUrl) {
  if (favorites[itemUrl]) {
    ...
    // Show Removed Confirmation fo 2seconds
    removedConfirmed.classList.remove("hidden");
    setTimeout(() => {
      removedConfirmed.classList.add("hidden");
    }, 2000);
